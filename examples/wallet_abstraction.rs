            ...
            None => {
                ...
                match withdraw {
                    ...
                    AdvanceWithdraw::ERC721 { token, id } => {
                        let is_owner =
                            env.erc721_owner_of(token, id).await == Some(metadata.sender);
                    ...
                    AdvanceWithdraw::ERC1155 { token, ids, data } => {
                        ...
                        if ids_balance.is_empty() {
                            ...
                            env.erc1155_withdraw(metadata.sender, token, ids_balance, data)
                                .await?;
                        }
                    }
                }
    ...
    async fn inspect(
        ...
        match inspect {
            ...
            InspectBalance::ERC20 { address, token } => {
                ...
                println!(
                    "Balance of ERC20 token {} for address {}: {}",
            ...
            InspectBalance::ERC721 { token, id } => {
                ...
                env.send_report(owner.to_owned().unwrap_or(Address::zero()))
                    .await?;
            }
...
mod tests {
    ...
    async fn test_erc20_deposit_and_withdrawal() {
        ...
        assert!(advance_result.is_accepted(), "Expected Accept status");
        assert_eq!(
            tester.erc20_balance(address, token_address).await,
            Uint::zero()
        );
        assert_eq!(advance_result.get_outputs().len(), 1);
    }
    ...
    async fn test_erc1155_deposit_and_withdrawal() {
        ...
        assert_eq!(
            tester
                .erc1155_balance(address, token_address, token_id)
                .await,
            amount
        );
