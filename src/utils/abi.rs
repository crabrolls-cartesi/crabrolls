pub mod abi {
    ...
    pub mod extract {
        ...
        pub fn int(arg: &ethabi::Token) -> Result<Uint, Box<dyn Error>> {
            arg.clone()
                .into_int()
                .ok_or_else(|| "invalid type for Int".into())
        }

        pub fn bool(arg: &ethabi::Token) -> Result<bool, Box<dyn Error>> {
            arg.clone()
                .into_bool()
                .ok_or_else(|| "invalid type for bool".into())
        }
        ...
        pub fn bytes(arg: &ethabi::Token) -> Result<Vec<u8>, Box<dyn Error>> {
            arg.clone()
                .into_bytes()
                .ok_or_else(|| "invalid type for bytes".into())
        }
    ...
    pub mod utils {
        ...
        pub fn size_of_packed_token(token: &Token) -> usize {
            match token {
                ...
                Token::Array(tokens) | Token::FixedArray(tokens) => {
                    tokens.iter().map(size_of_packed_token).sum()
                }
        ...
        pub fn size_of_packed_tokens(tokens: &[Token]) -> usize {
            tokens
                .iter()
                .fold(0, |acc, token| acc + size_of_packed_token(token))
        }
    }
            ...
            for param in params {
                match param {
                    ...
                    ParamType::Uint(size) | ParamType::Int(size) => {
                        ...
                        ensure_payload_length(
                            &payload,
                            byte_size,
                            &format!("Uint/Int of size {}", size),
                        )?;
                    ...
                    ParamType::FixedBytes(size) => {
                        ensure_payload_length(
                            &payload,
                            *size,
                            &format!("FixedBytes of size {}", size),
                        )?;
                    ...
                    ParamType::Bytes | ParamType::String => {
                        ...
                        if let ParamType::Bytes = param {
                            ...
                            tokens.push(Token::String(String::from_utf8(
                                payload[32..32 + size].to_vec(),
                            )?));
                        }
...
mod tests {
    ...
    fn test_generic_encode_function_call() {
        ...
        let encoded =
            abi::encode::function_call(abi_json, function_name, params).expect("encoding failed");
